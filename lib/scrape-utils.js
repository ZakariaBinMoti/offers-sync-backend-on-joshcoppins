 ...
 */
async function fetchHtmlViaProxy(url, { timeoutMs = 50000 } = {}) {
  ...
  //
  // render=false: cheaper (1 credit) and faster. Works for most sites.
  // If Yamaha's Incapsula challenge requires JS execution even via the proxy,
  // change this to render=true (5 credits/request, still ~155/month on free
  // tier for a once-daily cron — well within the 1,000/month limit).
  const proxyUrl =
    `https://api.scraperapi.com/?api_key=${apiKey}` +
    `&url=${encodeURIComponent(url)}` +
    `&country_code=nz` +
    `&render=true`; // Incapsula requires JS execution to pass its challenge
...
async function fetchDebugInfo(url) {
  ...
  try {
    ...
    // A large response (>10 KB) that only matches weak/common keywords like
    // 'captcha' (often found in normal page markup as a hidden field) is most
    // likely a false positive — the page is real content, not a block page.
    // A genuine block page is always tiny (typically <2 KB).
    const WEAK_SIGNS = ['captcha', 'blocked'];
    const strongMatches = matchedBlockSigns.filter((s) => !WEAK_SIGNS.includes(s));
    const likelyBlocked =
      strongMatches.length > 0 || (matchedBlockSigns.length > 0 && text.length < 10000);
